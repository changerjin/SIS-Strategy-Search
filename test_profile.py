...
def pump(g6k, tracer, kappa, blocksize, dim4free, down_sieve=False, # Main parameters
         goal_r0=None, max_up_time=None, start_up_n=50, saturation_error="weaken",  # Flow control of the pump
...
def bkz_tour(g6k, tracer, blocksize, last_svp):
    """
    ...
    # for i, (kappa, beta, f) in enumerate(indices):
        ...
        flast = pump(g6k, tracer, kappa, beta, f)
        if flast == -1:
            # g6k.M.B = copy.copy(Last_Mat)
            # g6k.update_gso(0, d)
            return -1
        new_r0rate = get_r0rate(g6k, kappa, beta)

        print("\tr0/gh: %.6f, current rate: %.3f ==> %.3f\n" % (get_r0rate(g6k, 0, last_svp), last_r0rate, new_r0rate))
    ...
    print("bkz-%d: average time: %.3f" % (blocksize, avg_time))
...
def sis_kernel(arg0, params=None, seed=None):
    ...
    params['saturation_ratio'] = 0.5
    params['db_size_factor'] = 3.2
    ...
    # 需要严格比目标小
    print("start solving sis challenge-%d" % n)
    ...
    # BKZ
    block_index = 0
    while(block_index < len(blocksizes)):
    # for blocksize in blocksizes:
        blocksize = blocksizes[block_index]
        bkz_start_T = time.time()

        print ("Starting a BKZ-%d tour. " % (blocksize))        
        avg_time = bkz_tour(g6k, tracer, blocksize, s)
        if avg_time == -1:
            continue
    ...
    # Larger SVP
    # print("larger SVP")    
    # before_svp_r0rate = get_r0rate(g6k, 0, s)
    # f = default_dim4free_fun(s) #

    # print("Starting svp pump_{%d, %d, %d}, r0/gh: %.6f, target_norm: %d" % (0, s, f, before_svp_r0rate, target_norm))

    # pump(g6k, tracer, 0, s, f+4)
    # print("\n\n All done~~~\n")

    # print("before svp, r0/gh: %.6f, target_norm: %d" % (before_svp_r0rate, target_norm))
    ...
    if g6k.M.get_r(0, 0) < target_norm - 1e-3:
        print ("Finished! TT=%.2f sec" % (time.time() - T0))
        print (g6k.M.B[0])
        filename = 'lattice_challenge/%03d-solution.txt' % (n)
        fn = open(filename, "w")
...
# python3 test_profile.py 800 --threads 32 --gpus 1 2>&1 | tee logs/Test_Profile_800.log