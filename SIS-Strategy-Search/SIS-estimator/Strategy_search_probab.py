...
def setup_logging(tar_dim):
    ...
    log_name = f"log_Strategy/log_{tar_dim}_probab"
...
####################################################################### 模拟器 #############
def sim_bkz(r, block_size, last_svp):
    # logging.info("simulate bkz %d" % block_size)
    bsw18 = BSW18_simulate(r, BKZ.Param(block_size=block_size, max_loops=1))
    return bsw18[0]
...
def succ_bz_num(ori_r, bz, last_svp, target):
    ...
    while r0rate < 1.05**2:
        ...
        if r0rate < 1.001 * last_r0rate:
            # logging.info(f"r0rate not improving: {r0rate} < {1.001 * last_r0rate}")
            return -1
    return i
...
def best_succ_bz(ori_r, last_svp, target):
    ...
    for step in step_sizes:
        ...
        if bz > rng[-1]:
            logging.info("cannot find valid bz")
            return [], -1
        rng = [bz - step, bz]
    ...
    cost = pair[0][0]
    bzs = pair[0][-1]
    logging.info("last svp = %d, bz = %d, num = %d, cost = %.0f, svp cost = %.0f" % 
                 (last_svp, bzs[0], len(bzs), cost, complexity_sieve(last_svp)))
    return bzs, cost
...
def get_best_d(tar_dim, min_d, max_d, min_svp, max_svp):
    ...
    if best_d == -1:
        logging.info("cannot find best d in range [%d, %d)" % (min_d, max_d))
        exit(1)
    else:
...
def apply_bkz(ori_r, bzs, last_svp, target, verbose=True):
    ...
    if verbose:
        logging.info(f"done, overall cost {cost} {float(math.log(cost, 2))}")
    if r0rate < 1.05**2:
        if verbose:
            logging.info("fail!!!!!\n\n")
        return -1, rs
...
def get_best_bzs(tar_dim, d, cost_bound, init_bzs, last_svp, q, max_svp):    
    ...
    all_strategy = [(cost_bound, init_bzs, last_svp, d)]
    ...
    while len(suffixes) > 0:
        ...
        suffixes = suffixes[1:]
        logging.info(f"cur_suffix: {cur_suffix}")
        ...
        logging.info(f"first choice: { (cost, bz, n_bz) }")
    ...
    if len(all_strategy) > 0:
        ...
        logging.info(float(math.log(all_strategy[0][0], 2)))
        ...
        logging.info(f"{bzs} * {best_S[1][-1]}")
        logging.info(float(math.log(cost, 2)))
...
def SIS_startegy_search(target_dim, expect_d, expext_svp):
    min_svp = expext_svp - 10
    max_svp = expext_svp + 10
    min_d = expect_d - 10
    max_d = expect_d + 10

    setup_logging(target_dim)  # 初始化日志
    start = time.time()

    best_d, best_res, q = get_best_d(target_dim, min_d, max_d, min_svp, max_svp)
    get_best_bzs(target_dim, best_d, best_res[0], best_res[1], best_res[2], q, max_svp)
