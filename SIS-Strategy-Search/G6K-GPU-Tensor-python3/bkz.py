...
def load_whole_matrix(tar_dim):
    src_file = "lattice_challenge/challenge-%d" % (tar_dim)
...
def bkz_kernel(arg0, params=None, seed=None):
    ...
    A,q = load_whole_matrix(900)
    A = matrix_shrink(A, d)
    A = IntegerMatrix.from_matrix(A)

    MM = GSO.Mat(A, float_type="mpfr",
            ...
            with tracer.context("tour", t):
                if algbkz == "fpylll":
                    ...
                    pump_n_jump_bkz_tour(g6k, tracer, blocksize, jump=jump,
                                         dim4free_fun=dim4free_fun,
                                         extra_dim4free=extra_dim4free,
                                         pump_params=pump_params, goal_r0=1.05)
                else:
    ...
    print([g6k.M.get_r(i,i) for i in range(d)])
    filename = "qarychallenge/afterbkz-%02d-dim-%03d-seed-%02d.txt"%(blocksize, d, challenge_seed)
...
def bkz_tour():
    ...
    args, all_params = parse_args(description,
                                  ...
                                  bkz__blocksizes="75,75,79,117",
                                  bkz__pre_blocksize=0,
                                  bkz__tours=50,
