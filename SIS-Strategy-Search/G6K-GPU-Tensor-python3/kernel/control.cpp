...
void Siever::initialize_local(unsigned int ll_, unsigned int l_, unsigned int r_)
{
    ...
    double log_big_lattice_square_vol = 0;
    for (unsigned int i = ll; i < r; ++i)
    {
        log_big_lattice_square_vol += std::log(full_rr[i]);
    }
    // big_gh = std::exp((log_big_lattice_square_vol - log_ball_square_vol) / (1.0 * (r - ll)));
    // std::cout << "big gh: " << big_gh << std::endl;
...
void Siever::lift_and_replace_best_lift(ZT * const x_full, unsigned int const i)
{
    ...
    // CS改
    // if ((i == ll) && ((len_precise / big_gh) > 1.05 * 1.05)) std::cout << len_precise << "not satisfy 1.05" << std::endl;
    // else std::cout << "satisfy 1.05" << std::endl;
