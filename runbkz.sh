...
fileprefix=bkz_runs/bkz_${dim}_${tag}
mkdir bkz_runs 2>/dev/null
...
echo "Running: bkz.py $opts" | tee ${fileprefix}.cout${run}.log
...
python3 ./bkz.py ${opts} --workout/save_prefix ${fileprefix}.mat${run}  2>&1 | tee ${fileprefix}.cout${run}.log
